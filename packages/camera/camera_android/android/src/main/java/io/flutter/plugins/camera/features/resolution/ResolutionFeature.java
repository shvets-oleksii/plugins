...
public class ResolutionFeature extends CameraFeature<ResolutionPreset> {
  ...
  static Size computeBestPreviewSize(int cameraId, ResolutionPreset preset)
    ...
    if (Build.VERSION.SDK_INT >= 31) {
      ...
      if (videoProfiles == null || videoProfiles.isEmpty()) {
        //Fallback to legacy
        return computeBestPreviewSizeLegacy(cameraId, preset);
      }
  ...
  @VisibleForTesting
  static Size computeBestPreviewSizeLegacy(int cameraId, ResolutionPreset preset)
          throws IndexOutOfBoundsException {
  ...
  private void configureResolution(ResolutionPreset resolutionPreset, int cameraId)
    ...
    if (Build.VERSION.SDK_INT >= 31) {
      ...
      if (videoProfiles == null || videoProfiles.isEmpty()) {
        //Fallback to legacy
        configureResolutionLegacy(resolutionPreset, cameraId);
        return;
      }
      EncoderProfiles.VideoProfile defaultVideoProfile = videoProfiles.get(0);
      if(defaultVideoProfile==null) {
  ...
  private void configureResolutionLegacy(ResolutionPreset resolutionPreset, int cameraId)
          throws IndexOutOfBoundsException {
